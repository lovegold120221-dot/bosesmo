...
export function useLiveApi({
  ...
  useEffect(() => {
    ...
    const showSystemError = (text: string) => {
      ...
      // If the last turn was a generic system message, detailed error, or server state, consolidate it
      if (lastTurn && lastTurn.role === 'system' && (
        lastTurn.text.includes('Connection Closed') || 
        lastTurn.text.includes('terminated unexpectedly') ||
        lastTurn.text.includes('Connection failed') ||
        lastTurn.text.includes('Live API Error') ||
        lastTurn.text.includes('Quota Exceeded') ||
        lastTurn.text.includes('Server is down')
      )) {
        if (lastTurn.text.includes('Quota Exceeded') && text.includes('Live API Error')) {
          // Do not overwrite the cleaner "Quota Exceeded" message with a generic error
          return;
        }
        useLogStore.getState().updateLastTurn({ text });
      } else {
    ...
    const onError = (e?: any) => {
      ...
      if (isQuota) {
        ...
        console.warn("Live API connection closed or rate-limited:", errMsg || e);
      }
      ...
      if (isDepleted) {
        ...
        showSystemError(`⚠️ **Live API Error:** Connection failed or was terminated unexpectedly. Please verify your Eburon credentials and tier permissions.`);
      }
    };
