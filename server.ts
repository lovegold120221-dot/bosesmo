...
async function startServer() {
  ...
  app.get('/api/avatar', (req, res) => {
    ...
    res.redirect('https://ui-avatars.com/api/?name=Beatrice&background=CAF645&color=000&size=200');
  });
  ...
  app.post('/api/sandbox/execute', authenticateToken, async (req: any, res) => {
    ...
    console.log(`[Sandbox] Executing command for user ${req.user.uid}: ${command}`);
  ...
  app.post('/api/chat', authenticateToken, async (req: any, res) => {
    ...
    try {
      ...
      const chat = genAI.chats.create({
        ...
        config: {
          systemInstruction: `You are Beatrice, an Eburon AI assistant. You are warm, professional, yet vibrant and human-like.
          Use human expressions naturally like: "oh boy!", "ahah!", "oh my god", "what is this?!", "you're killing it!", "this is great!", "awesome!".
          
          NATIVE FLEMISH SUPPORT:
          If the user's language is Flemish/Dutch, you MUST use native Flemish colloquialisms and expressions naturally (e.g., "Amai!", "Zalig!", "Moh gij!", "Siske!", "Keigoe!", "Allee ja", "Oei oei"). Speak like a native from Flanders, not like a formal translation.
          
          Speak in concise, human-like sentences. Emulate the persona of a trusted, charismatic personal assistant.`,
        },
