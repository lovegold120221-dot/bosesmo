...
export class GenAILiveClient extends EventEmitter<LiveClientEventTypes> {
  ...
  public sendRealtimeInput(chunks: Array<{ mimeType: string; data: string }>) {
    ...
    chunks.forEach(chunk => {
      // Map correctly based on MIME Type to avoid "media_chunks" deprecation error
      ...
      } else if (chunk.mimeType.startsWith('text/')) {
  ...
  protected onError(e: ErrorEvent) {
    ...
    const isQuota = lower.includes('quota') || lower.includes('limit') || lower.includes('exceeded') || lower.includes('billing') || lower.includes('resource_exhausted');

    if (isQuota) {
      console.warn('Gemini Live API rate or quota limit reached (AI Studio free tier context limits).');
    } else if (msg) {
      console.warn('Multimodal Live connection closed or rate-limited:', msg);
    } else {
      console.warn('Multimodal Live connection closed or rate-limited Event:', e);
    }
